# GraphMind: LangChain & LangGraph Refactoring Plan

This document details the architectural plan to refactor the **GraphMind** codebase using **LangChain** and **LangGraph** to replace manual iteration loops, custom string parsing, and raw driver calls with structured state management, standard chain runnables, and modular graphs.

---

## 1. Consolidated Code & Imports
...
# --- 1. Custom Ollama Cloud Chat Model Wrapper ---
...
if "vector_store" not in st.session_state:
    st.session_state.vector_store = Chroma(
        ...
        embedding_function=HuggingFaceEmbeddings(
            ...
            model_kwargs={"device": "cuda" if torch.cuda.is_available() else "cpu"}  # CPU fallback
        )
    )
if "neo4j_graph" not in st.session_state:
    st.session_state.neo4j_graph = Neo4jGraph()
...
def visualize_thought_graph(path: List[str]):
    if not path: return
    nodes = []
    edges = []
    seen_urls = set()
    ...
    for i, url in enumerate(path):
        if url not in seen_urls:
            seen_urls.add(url)
            res = st.session_state.neo4j_graph.query(
                "MATCH (p:Page {url: $url}) RETURN p.title AS title", {"url": url}
            )
            label = res[0]["title"] if res else url
            nodes.append(Node(id=url, label=label, size=20, color="#1e88e5"))
        if i > 0:
            edges.append(Edge(source=path[i-1], target=url, label="EXPLORED"))
            
    agraph(nodes=nodes, edges=edges, device="mobile", height=300)
```

---

## 3. CLI Terminal Testing
...
if __name__ == "__main__":
    ...
    print("Initializing local testing connection clients...")
    embeddings = HuggingFaceEmbeddings(
        model_name="BAAI/bge-large-en-v1.5",
        model_kwargs={"device": "cuda" if torch.cuda.is_available() else "cpu"}  # CPU fallback
    )
    db = Chroma(collection_name="metakgp_wiki", persist_directory="VectorStore", embedding_function=embeddings)
    graph = Neo4jGraph()
    ...
    print("Executing refactored LangGraph engine on query...")
    result = app.invoke(inputs, config=config)
    print("\n--- CLI RUN RESULTS ---")
    print("Final Answer:\n", result.get("final_answer"))
    print("\nThought Traversal Path:\n", result.get("thought_path"))
```
...
```bash
pip install langchain langchain-community langchain-ollama langchain-huggingface langgraph
```
